...
# Backup Unix directories on mx1 /export/projects
  $time = gettime();
  $startTime = time();
  print FH  "Backup of mx1:/export/projects started at $time.\n";
  $command = "rsync -a --delete --stats --exclude='*.trn' --exclude=simulation/ /export/projects $destdir";
  print $command;
  if(Utils::execSys($command,\@message)) {
    print FH  "rsync of mx1:/export/projects returned non zero.\n";
    print FH @message;
  }
  $elapsedTime = diffTime($startTime);
  print FH "Backup of mx1:/export/projects complete: Elapsed time $elapsedTime\n"; 
